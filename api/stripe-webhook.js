...
// charge / transfer / application-fee / on_behalf_of that the verified payment RPC validates.
async function stripeGetPaymentIntent(piId) {
  if (!STRIPE_SECRET_KEY || !piId) return null;
  const url = `https://api.stripe.com/v1/payment_intents/${encodeURIComponent(piId)}?expand[]=latest_charge`;
  const r = await fetch(url, { headers: { Authorization: 'Bearer ' + STRIPE_SECRET_KEY } });
  if (!r.ok) { console.error('stripe PI retrieve failed', piId, r.status); return null; }
  return r.json();
}
...
async function handleCheckoutSessionExpired(event) {
  ...
  const _r = await sbRpc('expire_payment_attempt', { p_session_id: s.id });
  const val = Array.isArray(_r) ? _r[0] : _r;
  console.log('checkout.session.expired:', s.id, (val && val.outcome) || 'no_attempt');
}

// Shared: converge ONE Stripe refund object onto its ledger request via the verified-event RPC.
// Resolves by TRUSTED metadata.refund_request_id first (survives a webhook that beats our own id
// persistence), validates PI/charge/amount/currency ownership, and quarantines only a truly
// unknown refund. A THROW (reservation underflow / amount mismatch) is NOT swallowed -> the event
// is marked failed and Stripe retries. (R10-P0-5)
async function applyRefundObject(rf, charge, eventId) {
  const metaReqId = (rf.metadata && rf.metadata.refund_request_id) || null;
  const pi = rf.payment_intent || (charge && charge.payment_intent) || null;
  const chId = rf.charge || (charge && charge.id) || null;
  const _r = await sbRpc('apply_refund_event', {
    p_refund_id: rf.id, p_status: rf.status || 'succeeded', p_amount: rf.amount, p_currency: rf.currency,
    p_pi: pi, p_charge: chId, p_meta_request_id: metaReqId, p_event_id: eventId || null,
  });
  const val = Array.isArray(_r) ? _r[0] : _r;
  const outcome = val && val.outcome;
  if (outcome === 'unmatched') {
    console.warn('unmatched stripe refund quarantined:', rf.id);
    try { await sb('unmatched_stripe_refunds', { method: 'POST', body: JSON.stringify({ stripe_refund_id: rf.id, stripe_charge_id: chId, stripe_payment_intent_id: pi, amount: rf.amount, currency: rf.currency, raw: rf }) }); } catch (_) {}
  } else if (outcome === 'contradiction' || outcome === 'failed') {
    console.warn('refund event', outcome, 'for', rf.id, val && val.reason); // RPC already opened a reconciliation_cases row
  }
  return outcome;
}

// Individual Refund lifecycle events (refund.created/updated/failed): verified-event application.
async function handleRefundEvent(event) {
  const rf = event.data.object || {};
  await applyRefundObject(rf, null, event.id);
}
...
async function handlePaymentIntentSucceeded(event) {
  ...
  await promoteBookings(bookingIds, { piId: pi.id, ledgerPaid: false });
  console.log(`payment_intent.succeeded (legacy): promoted ${bookingIds.length} booking(s)`);
}

// Promote a set of just-paid bookings: pending_payment → confirmed/pending, materialise the demo on
...
async function handlePaymentIntentFailed(event) {
  ...
  if (_gid) {
    try { await sb(`payment_attempts?stripe_payment_intent_id=eq.${encodeURIComponent(pi.id)}&status=eq.open`, { method: 'PATCH', body: JSON.stringify({ last_error: String(err).slice(0, 300) }) }); } catch (_) {}
    console.log('payment_intent.payment_failed (ledger group, kept payable):', pi.id, err);
    return;
  }
  const bookingIds = bookingIdsFrom(pi.metadata);   // legacy pre-ledger bookings only
...
async function handleChargeRefunded(event) {
  ...
  for (const rf of refunds) {
    await applyRefundObject(rf, charge, event.id);   // reconciliation-only; each refund -> its own request
  }
}
...
async function handleCheckoutSessionCompleted(event) {
  ...
  // ---- Payment mode: THE verified ledger fulfilment path (R10-P0-1/P0-2) ----
  ...
  if (mode === 'payment') {
    ...
    if (!pi) throw new Error('cannot retrieve PaymentIntent for session ' + session.id);   // retryable
    const charge = (pi.latest_charge && typeof pi.latest_charge === 'object') ? pi.latest_charge : null;
    const chargeId = charge ? charge.id : (typeof pi.latest_charge === 'string' ? pi.latest_charge : null);
    const applied = await sbRpc('apply_verified_payment', {
      p_session_id: session.id, p_payment_intent: pi.id, p_charge: chargeId,
      p_amount: session.amount_total != null ? session.amount_total : pi.amount_received,
      p_currency: session.currency || pi.currency,
      p_connect_dest: (charge && charge.destination) || pi.on_behalf_of || null,
      p_on_behalf_of: pi.on_behalf_of || null,
  ...
  // Only subscription-mode continues below (Pro upgrade mirroring).
