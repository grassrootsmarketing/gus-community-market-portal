...
- **Release C refund rule — Option 2 selected 2026-09-10 (Codex recommendation on Claude's options packet; David to confirm before production activation):** brand-initiated cancellation is non-refundable from purchase (slot released, no refund, no credit); retailer-caused cancellation (decline / cannot host) refunds the full remaining amount attributable to that demo only (siblings untouched); hard 14-day minimum lead universal; existing bookings keep their accepted terms (policy versioned prospectively); one-time retailer-discretion courtesy reschedule pass (same booking/location/price/duration, issuable once confirmed+paid, one lifetime use, no money movement); 14-day store notice is an operational planning checkpoint, not a refund cutoff. No new 24-hour refund delay, no store credits. Codex checklist correction: C-06 atomic cancellation and C-07 grouped-payment isolation are RETAINED in full. Source: `Documents/Codex/2026-07-21/github-plugin-github-openai-curated-remote/Demohub-Release-C-Option-2-Handoff-for-Claude.md`. Supersedes the 2026-09-09 addendum's refund-window rule. Not part of Release A.
