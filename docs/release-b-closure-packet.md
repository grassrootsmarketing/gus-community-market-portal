...
| **Post-freeze merge (2026-09-11 10:20Z)** | Production moved to `9fa0854` (hotfix: the signup category is persisted once at redeem and a first booking saves it to the profile, so the booking form stops asking "What are you demoing?"; `api/brand-signup.js`, `r/gus/index.html` +3 lines, `tests/brand_signup` 21/21). `main` was merged into the branch with no conflicts → **deployable head `ba4909e`** = `778ef1e` + that hotfix. Per Codex's stance that verification follows the change (no schema change, three lines on the public page outside the slot code), the suites job was re-dispatched on `ba4909e`; the clean build and staging passes on `778ef1e` stand. Every proof in this packet was taken on `778ef1e`. |
...
| Containment | Gus only, signup OFF, capacity 1, no viewers, support OFF. **Change 2026-09-11 (David's decision, recorded here on purpose):** `PROVISIONAL_HOLDS_ENABLED=true` in Vercel Production on the current build `1806de4` — the hard COI gate was a real friction point for brands, and the 24-hour hold (validated live 2026-09-01 both paths; adversarial suite in CI) removes it: book immediately, card authorized not charged, capture on COI approval within 24h, else $0 release. Verified from the public status probe: `provisional-sweep` reports `required: true` and healthy. Release B does not touch the hold code. |
