# Demohub — Release B production cutover runbook (operator-run; contained prelaunch switch, v4.1)
...
The hold notice's retry payload is not yet frozen (deferred follow-up F-1), so a held-notice fulfilment can end `failed` with an open `fulfil:<booking>` case even though the provider **may already have accepted and delivered** the first attempt. When the named operator finds one: (1) check Resend's message history for that recipient and key (`hold-placed:<booking>:<generation>`); (2) check the booking's current state — still held, captured, released, expired; (3) only then decide. Do not blindly resend a stale hold notice, do not rotate or invent a provider key to get past the conflict, and never start a payment, capture, release or refund as a way of recovering an email. If a replacement message is warranted, it is a deliberate operator action: written for the booking's current state, sent by hand, and recorded on the case before it is resolved.
