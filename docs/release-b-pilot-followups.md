...
| B | Brand dashboard Overview tile says "No COI on file" while a certificate is pending review. Narrow display patch: a "COI status" tile driven by the existing `getBrandStatus(profile)` mapping (none / pending review / current / expiring / expired); focused UI tests; **no** change to approval, capture, expiry or auto-cancel | Claude (branch) → David says "deploy" | **patch ready, not deployed:** branch `fix/coi-status-tile` @ `29a5362` (= live `67613b5` + 2 files: `brand/dashboard/index.html` +16/−1, new `tests/brand_coi_tile_dom.e2e.mjs`); focused UI test 21/0 across none / pending review / current / expiring / expired, tile and Compliance strip agree, pending never reads as approved; `npm run check` + `npm test` green |
