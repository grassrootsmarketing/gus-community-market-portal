...
- **Corrected candidate `ad4f1ed`** = `f7d1297` + eight commits: `3975a43` R4-01 (migration **0081**), `f6754d9` R4-02, `19bafa1` R4-03, `9dffc94` R4-04 CI rehearsal + teardown, `3b27159` a one-line no-undef fix (`globalThis.AbortSignal`) found by `npm run check` on the first frozen tree, `8cb8c8e` the diagnosed reschedule fixture (below), `1ad84fe` `tests/mail_deadline.test.mjs` given a fixture binding so the offline CI `suites` job can run it, and `ad4f1ed` migration **0082** — the correction for the defect the upgrade rehearsal found on its first CI run (below). Proofs are labelled with the SHA they ran on; the battery and the e2e journeys are re-run on `ad4f1ed` (§5).
...
- **The rehearsal earned its place on its first run.** On a database reset to production's ledger (0072, 0073 absent) with seeded held / paid-awaiting / confirmed rows, `supabase migration up` 0074–0081 applied cleanly and the ledger tail was exact — and then `verify-post.sql` raised: two of the three pre-existing bookings had **no** `start_at/end_at/timezone/duration_hours`. Root cause in the migration text: 0074's backfill is scoped to `pending`/`confirmed` and 0075 copies `duration_hours` only from an existing demo, so a `held` or `pending_payment` booking present at cutover keeps a NULL snapshot; `snapshot_drift()` cannot see it (it audits stamped rows only), and when such a row later transitions the snapshot trigger fills in the 3-hour default — a 1-hour hold would have become a 3-hour demo. Neither a clean build nor any suite could show this. **Migration 0082** (forward-only, idempotent) stamps every active booking from its venue's slot configuration on its own date (fallback: linked demo, then the trigger default, counted in its NOTICE) and asserts none is left unstamped; the rehearsal now also asserts the seeded held and paid rows carry their 1-hour slot length and that the projected demo does. Production has no bookings today, so the defect had no live victim; it would have had one at the first cutover with a brand mid-hold.
- **R4-04**: the runbook is rewritten as a verified quiet cutover (Part C) — the removed RPC is no longer called a fence, holds-OFF is no longer called an intake pause, the preflight has the stop conditions you named, exact versions are verified then recorded, the generation-1 wrapper is withdrawn, recovery is forward-only — and the upgrade path itself is rehearsed by a new CI job (`upgrade-rehearsal`): reset staging to 0072 with 0073 absent, seed production-shaped rows through the ledger RPCs, `supabase migration up` 0074–0082, verify ledger/data/snapshots/audits/contracts/runtime, and require the upgraded schema manifest to equal a clean build's.
...
- Gates: see Part B §8 — on `1ad84fe` David approved and the suites, clean build and both staging passes were green while the rehearsal failed on the finding above; the full set is re-dispatched on `ad4f1ed` (run 34715303105) and needs David's `staging` approvals again.
...
1. Accept or reject Release B on `ad4f1ed` once the four gate results are appended to §8.
2. Confirm the runbook's entry/exit conditions (Part C §1, §6) and the rehearsal's scope (Part C §7).

---

# Part B — Closure packet (round 4 corrections: R4-01 … R4-04 on `ad4f1ed`)
...
| Item | Value |
|---|---|
| **Candidate SHA** | `ad4f1ed` (branch head) |
| Ancestry | `f7d1297` (your reviewed candidate) ← `3975a43` R4-01 ← `f6754d9` R4-02 ← `19bafa1` R4-03 ← `9dffc94` R4-04 CI rehearsal + route_flows teardown ← `3b27159` `globalThis.AbortSignal` ← `8cb8c8e` reschedule fixture (test-only) ← `1ad84fe` mail_deadline fixture binding (offline test only) ← `ad4f1ed` migration 0082 + rehearsal assertions (rehearsal finding) |
| Migrations | 0000–**0082** (83 SQL files + README). New: `0081_fulfillment_terminalization_fence.sql` (R4-01), `0082_backfill_active_booking_snapshots.sql` (rehearsal finding). CI `EXPECTED_MIGRATIONS: '83'`. Forward-only; 0074–0080 untouched. |
| Test DB | demohub-rebuild-check (`tileejdviuvijumjeplv`): ledger rows `0073`–`0082`; 0081 and 0082 applied one file each with their post-condition blocks (0082 met zero unstamped rows there — the CI final reset had emptied it; its logic was dry-run on simulated pre-0074 rows in a rolled-back transaction: 1 h / 1 h / 3 h stamped, drift 0) |
...
| **Rehearsal finding → migration 0082** (held / `pending_payment` bookings left without a snapshot by 0074/0075) | **Closed** — forward-only migration | `0082_backfill_active_booking_snapshots.sql`: for every active booking (`pending`, `confirmed`, `held`, `pending_payment`) with a missing snapshot or duration, resolve the duration from the venue's slot configuration on that date via `venue_slot_offered` (the write-time rule), fall back to a linked demo's duration, else the trigger default — counted in the NOTICE, never raised on unvalidatable data; stamp `timezone` from the retailer, `start_at` via `booking_slot_start(…, 11)`, `end_at` from the duration, in one UPDATE that the snapshot trigger recomputes identically. Post-conditions: no active parseable row unstamped; `end_at` consistent with duration. Rehearsal `verify-post.sql` now also asserts the seeded held and paid rows are 1 h in `America/Los_Angeles` and that the projected demo is 1 h, not 3. | First rehearsal run **34689957112**: reset → seed → `migration up` 0074–0081 ✓ → ledger tail exact ✓ → `verify-post` **raised** `2 pre-existing booking(s) were not stamped` (artifact `failed-upgrade-rehearsal-diagnostics`). Rolled-back dry run of 0082 on simulated pre-0074 rows (test DB): held 11:00 → 1 h, `pending_payment` 14:00 on a 3 h slot → 3 h, confirmed 10:00 → 1 h, `snapshot_drift` 0. Second rehearsal run on `ad4f1ed`: §8. |
| **R4-04 (P1 operational)** unsafe cutover/recovery instructions | **Closed** — runbook rewritten + CI rehearsal | Part C. Corrections 1–8 in order: the removed RPC is described as what it is (old workers act before completion; retry repeats); holds-OFF and checkout-OFF described precisely; a verified quiet cutover under David's approval (flags on the old build, operator freeze, cron-gap timing, and a preflight that must show zero open attempts / unsettled groups / leased rows / refund work / webhook events mid-handler / holds expiring — re-read immediately before the first migration, every nonzero row explained, never cleared by approving/declining); exact versions 0074–0081 each with a verify statement before its ledger row, 0073 explicitly absent; the generation-1 wrapper withdrawn with the reason; forward-only recovery (emergency build = the candidate with optional switches off; never redeploy `53961d7` after 0078, never reset generations or wipe); the rehearsal (below); evidence to file. The Stripe endpoint stays enabled; the inbox lease + 5xx-on-failure keep events durable. | **CI job `upgrade rehearsal 0072 -> 0081 (staging)`** (`verify.yml`, input `upgrade_rehearsal`, staging-only refusals, `staging` approval): hide every migration ≥ 0073 → `db reset` → `supabase/rehearsal/verify-pre.sql` (ledger max 0072, 0073 absent, six-arg RPC present, no `generation`, no `booking_transition`/`record_fulfillment`) → `seed-pre-0074.sql` (retailer/venue/brand; confirmed booking + linked demo; held booking authorized through `checkout_claim_group`/`register_payment_attempt`/`apply_verified_authorization`; paid booking through `apply_verified_payment` left undrained; `_open_case`; `processed_stripe_events`; `cron_heartbeat`) → un-hide 0074–0082 (0073 stays hidden) → `supabase migration up --linked --yes` → `verify-ledger.sql` (tail exactly `0074,…,0082`) → `verify-post.sql` (3 bookings preserved with `start_at/end_at/timezone/duration_hours` stamped; held row still authorized; paid row still `pending_payment`/paid; every pre-existing outbox row pending/generation 1/unleased; groups authorized+paid; case open; event present; five audits 0; contracts; runtime: claimless record → stale, claim returns generation 1, `promote_paid` then `confirm` project exactly one linked demo, wrong-generation record → stale) → upgraded manifest → restore all files → final reset + identity (always) → clean manifest → **upgraded == clean** required (ledger row count normalized: 0073). Artifact `upgrade-rehearsal-evidence`. Locally, seed + verify-post were executed inside a rolled-back transaction on the test DB (current schema, so a syntax/contract check only — labelled as such); the real proof is the CI run in §8 — and the first one proved the point by failing on 0074/0075's scope (row above). |
...
- **0082 fallback order** is slot configuration → linked demo → trigger default (3 h), with counts in the NOTICE; a row this migration cannot resolve is stamped with the same defaults the trigger would later use, so nothing is invented that the runtime would not also invent, and the audits report it afterwards.
...
7. `ad4f1ed` (0082 + rehearsal assertions): 0082 applied to the test DB with its ledger row; `npm run check` (83 migrations) ✓; rehearsal seed + verify-post dry-run (rolled back) ✓; full battery on `ad4f1ed` (`battery9.log`): all eight sections green (12:50–13:06Z: check 83 migrations ✓ · unit 22 ✓ · columns ✓ · ledger 12/63/28/**90** ✓ · routes 191/75+30/117/**84**/45/35/125/28/78/19 ✓ · capacity 35/38/97/19/117 ✓ · live 11/21 ✓ · reschedule **51/51** ✓); DOM/agreement/Stripe e2e on `ad4f1ed`: DOM **52/52**, agreement **18/18**, Stripe test-mode journey **88/88**; audits after all of it: projection 0, offering-invariant 0, drift 0, mismatches 0, capacity 0, fixture leftovers 0.
8. Probes: all three of yours reproduce at `f7d1297`; the working-tree variants (same harness shape, corrected contracts modelled) do not reproduce. Memory-harness evidence, labelled.
...
| Gate | Status |
|---|---|
| CI run on `1ad84fe` | **34689957112** — https://github.com/grassrootsmarketing/gus-community-market-portal/actions/runs/34689957112 : suites ✓ ubuntu (14 s) ✓ windows (33 s); clean build A/B ✓ (1 m 21 s); staging pass 1 ✓ (14 m 27 s); staging pass 2 ✓ (7 m 44 s); **upgrade rehearsal ✗** at `verify-post` — the 0074/0075 snapshot finding (§2). Earlier runs 34683529780 (`f7d1297`) and 34689862769 (`8cb8c8e`, suites failed because `mail_deadline` needed env) were cancelled/superseded. |
| CI run on `ad4f1ed` (`clean_build=true`, `staging_gate=true`, `upgrade_rehearsal=true`) | **34715303105** — https://github.com/grassrootsmarketing/gus-community-market-portal/actions/runs/34715303105 — suites ✓ both OS; clean build, staging pass 1/2 and the rehearsal await David's `staging` approval |
| suites (ubuntu, windows) on `ad4f1ed` | ✓ ubuntu (16 s), ✓ windows (34 s) |
| clean build A/B on `ad4f1ed` | awaits David's `staging` approval (was ✓ on `1ad84fe`; 0082 changes the schema chain, so it must run again) |
| staging pass 1, pass 2 on `ad4f1ed` | await David's `staging` approval (were ✓ on `1ad84fe`) |
| upgrade rehearsal 0072 → head on `ad4f1ed` | awaits David's `staging` approval — the run that must now pass `verify-post` with 0082 in the chain |
...
_Verbatim copy of `release-b-cutover-runbook.md` (demohub-docs commit `2e0ee12`)._
