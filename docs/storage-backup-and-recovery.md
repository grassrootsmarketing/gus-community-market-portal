...
| Retention | **Decided by David 2026-09-20 (Codex closure item 5, option B): a finite recovery window.** Off-machine (S3): snapshots are recoverable for **30 days**, then expire on a timer — there is **no** promise that the last good off-machine copy is kept forever. Control: the independent alert fires within 26 hours of a failed or missed backup; David responds to alerts promptly — the remaining days are a buffer, not a waiting period. On the workstation only: local pruning never removes the last good local snapshot. 7-day governance Object Lock on the bucket. |
...
- **Encrypted before it touches the backup folder.** Format: [age](https://github.com/FiloSottile/age) v1 (X25519 + ChaCha20-Poly1305), produced by the maintained age tool itself (pinned version). The scheduled job holds only the **public** recipient. Object names and the manifest are inside the encrypted file; local file names are opaque hashes.
...
- **Retention — a finite recovery window (David, 2026-09-20).** Off-machine copies exist for 30 days and then expire; nothing claims "never lose the last off-machine copy". How S3 does it: at 30 days the current version gets a delete marker (normal `offsite-list` / `offsite-fetch` stop finding it), and the noncurrent version is permanently removed 7 days later; during those 7 days only the account administrator (not the daily uploader) could still recover it by version id. If backups stop, the alert reaches David within 26 hours and he acts on it promptly; the remaining ~29 days are a buffer, not a waiting period. The local folder separately never prunes its last good snapshot, never prunes while backups are failing, and never prunes a snapshot without a confirmed off-machine copy. This is an engineering recovery window, not a legal, contractual, insurance or financial-records retention decision; no separate never-expiring anchor copy is kept, so customer documents are not retained indefinitely in the backup.
...
2. **Encryption tool — done 2026-09-20.** New snapshots are produced by the maintained `age` tool (pinned 1.3.1; a missing or different version fails the backup). It decrypted the oldest existing snapshot 12/12 and every encrypted path record (`evidence/backup/2026-09-20-age-reference-tool-interop.md`). Standard recovery on any computer: `age -d -i <recovery key file> <snapshot>.tar.age > snapshot.tar`, then unpack the tar; `manifest.json` maps the hashed file names back to bucket paths (the `restore` command does both).
3. **S3 create-only enforcement — done 2026-09-20.** The uploader is denied any write without `If-None-Match`; synthetic probe before/after: unconditional overwrite 200 → 403 (`evidence/backup/2026-09-20-s3-create-only-enforcement.md`).
4. **Retention model — decided 2026-09-20: option B, finite recovery window** (see "Privacy, access, retention"). The contradictory "never the last good off-machine copy" wording is removed. If a backup alert fires: open Claude and say "the Demohub backup alert fired"; fix the cause or run `cli.mjs daily` manually the same day; if it cannot be fixed within a few days, make a manual copy of the newest good snapshot to another approved location before the 30-day window matters.
