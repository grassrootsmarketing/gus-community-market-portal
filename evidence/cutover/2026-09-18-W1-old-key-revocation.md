...
## Addendum — 2026-09-19T00:50Z (post-launch housekeeping)
- David deleted the two remaining pre-rotation secret keys on the test project (`github_actions`, `vercel_preview`); the project's Secret keys list now holds only `rotated_2026_09` (dashboard, David). Afterwards: rotated key → HTTP 200; Preview operator probe `binding_error: null` (identity verified with the service key); production unaffected (`67613b5`, status operational — production uses a different Supabase project).
- The operator workstation's Supabase CLI link to production was removed: `demohub/supabase/.temp` set aside as `.temp.unlinked-from-prod-2026-09-19` (kept, excluded from git locally). A `--linked` command now fails as "not linked" instead of targeting production. Staging resets continue to go through the guarded CI jobs.
