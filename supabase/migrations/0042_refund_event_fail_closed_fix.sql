-- 0042_refund_event_fail_closed_fix.sql — corrective re-declare of apply_refund_event.
-- 0041 referenced fields on a possibly-unassigned RECORD when an event carried no metadata id,
-- raising 55000 instead of returning a clean 'unmatched'. Uses an explicit FOUND flag now.
-- (Original 0041 header follows.)
-- Codex Round 12 P1-3.
-- apply_refund_event was still conditionally validating: it skipped PI/charge/amount/currency
-- comparison whenever the caller passed null, treated any unrecognised status as "pending", and
-- used greatest(0, …) on failed/canceled — which silently masked reservation underflow (i.e. real
-- ledger corruption). Now: every Stripe identity/money field is REQUIRED, status is an enum, and
-- underflow/mismatch opens a durable case and mutates NO balance.
BEGIN;
...
DECLARE v_req record; v_alloc record; v_all boolean; v_terminal boolean; v_case uuid; v_status text; v_found boolean := false;
BEGIN
  ...
  -- resolve by TRUSTED metadata first, else by refund id
  ...
  IF NOT v_found THEN
    ...
    v_found := FOUND;
  END IF;
  IF NOT v_found THEN RETURN jsonb_build_object('outcome','unmatched'); END IF;   -- genuinely unknown refund
  ...
  -- ---- unconditional ownership + money validation (no null escapes) ----
  ...
  IF v_alloc.charge_id IS NULL OR p_charge <> v_alloc.charge_id THEN
    v_case := _open_case('payment_contradiction','refund-charge:'||p_refund_id,'refund_charge_mismatch', v_alloc.gid, v_req.id, v_req.parent_operation_id,
                         NULL, p_pi, p_charge, p_refund_id, p_amount, p_currency,
                         jsonb_build_object('event_charge',p_charge,'group_charge',v_alloc.charge_id));
  ...
  IF v_terminal THEN
    IF (v_status = 'succeeded' AND v_req.status = 'succeeded')
       OR (v_status IN ('failed','canceled') AND v_req.status IN ('failed','failed_terminal','canceled')) THEN
  ...
  -- ---- reservation sufficiency asserted on BOTH releasing paths (no greatest(0,…) masking) ----
  IF v_status IN ('succeeded','failed','canceled') AND v_alloc.reserved_refund_amount < v_req.amount THEN
    ...
    RETURN jsonb_build_object('outcome','contradiction','reason','reservation_underflow','case_id',v_case);  -- zero balance mutation
  END IF;
