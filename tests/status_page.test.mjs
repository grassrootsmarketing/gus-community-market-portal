...
const operational = {
  ...
  checks: {
    db: { ok: true },
    cron: { ok: true, jobs: { 'refund-worker': { ok: true, required: true }, 'provisional-sweep': { ok: true, required: true }, 'notification-worker': { ok: true, required: true }, daily: { ok: true, required: true } } },
    errors: { ok: true },
  },
...
// 1. Operational payload: all rows green, banner operational, no undefined/never/NaN.
{
  ...
  check('rows: db, cron, four job rows, errors', JSON.stringify(keys) === JSON.stringify(['db', 'cron', 'job:refund-worker', 'job:daily', 'job:provisional-sweep', 'job:notification-worker', 'errors']), keys.join(','));
  check('all rows are ok (green)', m.rows.every(r => r.tone === 'ok'), m.rows.map(r => `${r.key}=${r.tone}`).join(' '));
  const names = m.rows.map(r => r.name);
  check('job rows are labelled Refund processing / Daily tasks / Provisional holds sweep / Demo notifications',
    names.includes('Refund processing') && names.includes('Daily tasks') && names.includes('Provisional holds sweep') && names.includes('Demo notifications'), names.join(' | '));
...
// 2. Degraded with refund-worker ok:false → that job row red, cron row red, banner degraded.
{
  ...
  check('other job rows stay green', m.rows.filter(r => ['job:daily', 'job:provisional-sweep', 'job:notification-worker'].includes(r.key)).every(r => r.tone === 'ok'));
